...
def calcRowsForAlign(capacity, rows, dim):
    typesize=sys.getsizeof(float())
    ret=getBytes(capacity)/(rows*dim*typesize)
    return ret*rows
...
class DataHandler:
    ...
    """
    def __init__(self, inPath, outPath, indim, outdim, batchsize=100, numbatches=None, normalize=False,gpuMem='3G', mem='15G', writeBuffer='1G'):
        #init disk reader
...
class MemCache:
    ...
    """
    def __init__(self, diskReader, blocksize, dim, capacity):
        """
...
class DiskWriter:
    ...
    """
    def __init__(self, path, dim, batchsize=100, capacity='1G'):
